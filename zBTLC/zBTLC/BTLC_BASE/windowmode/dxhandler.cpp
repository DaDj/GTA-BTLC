...
#include "../../Patch/assembly.hpp"
#include "../../Patch/calling.hpp"

int CDxHandler::nResetCounter = 0;
int CDxHandler::nCurrentWidth = 0, CDxHandler::nCurrentHeight = 0;
int CDxHandler::nNonFullWidth = 1440, CDxHandler::nNonFullHeight = 900, CDxHandler::nNonFullPosX = 0, CDxHandler::nNonFullPosY = 0;
...
HRESULT(__stdcall *CDxHandler::oldReset)(LPDIRECT3DDEVICE9 pDevice, void* pPresentationParameters);
HRESULT(__stdcall *CDxHandler::oldSetViewport)(LPDIRECT3DDEVICE9 pDevice, CONST D3DVIEWPORT9* pViewport);
...
bool CDxHandler::bUseMenus = false;
bool CDxHandler::bUseBorder = true;
bool CDxHandler::btogglereq_btlc = false;
...
//	pParams->FullScreen_PresentationInterval = 0;
	...
	{
		dwWndStyle |= WS_POPUP;
		dwWndStyle &= ~(WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_THICKFRAME | WS_MAXIMIZEBOX);
...
void CDxHandler::MainCameraRebuildRaster(RwCamera* pCamera) 
{
	if (pCamera == *pRenderCamera) 
	{
		...
		(*pDirect3DDevice)->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pSurface);
		...
		*(int*)(RwD3D8AdapterInformation_DisplayMode + 4) = nGameHeight;
		...
		if (pCamera->frameBuffer && (pCamera->frameBuffer->width != nGameWidth || pCamera->frameBuffer->height != nGameHeight)) 
		{
		...
		if (pCamera->zBuffer && (pCamera->zBuffer->width != nGameWidth || pCamera->zBuffer->height != nGameHeight)) 
		{
...
HRESULT __stdcall ResetSA(LPDIRECT3DDEVICE9 pDevice, D3DPRESENT_PARAMETERS_D3D9* pPresentationParameters) {
...
HRESULT __stdcall SetViewport(LPDIRECT3DDEVICE9 pDevice, CONST D3DVIEWPORT9* pViewport) {

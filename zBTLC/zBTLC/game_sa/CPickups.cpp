...
void CPickups::DoPickUpEffects(CObject* entity) {
	...
	int Modelindex = entity->m_wModelIndex;


	int weirdTypeNumber = 0;
	CPickup *CurrentPickup = FindPickUpForThisObject(entity);
	...
	//Include weirtd corona stuff here --> for what is that exactly?

	//Check if pickup is a specifc model
	if (Modelindex == CHardModels::miAdrenaline
		|| Modelindex == CHardModels::miHealth
		|| Modelindex == CHardModels::miPropertyFsale
		|| Modelindex == CHardModels::miBonus
		|| Modelindex == CHardModels::miClosesp)
		weirdTypeNumber = 47;
	else if (Modelindex == CHardModels::miBodyarmor)
		weirdTypeNumber = 48;
	else if (Modelindex == CHardModels::miBigdollar)
		weirdTypeNumber = 53;
	else if (Modelindex == CHardModels::miBribe
		|| Modelindex == CHardModels::miInfo
		|| Modelindex == CHardModels::miKillfrenzy
		|| Modelindex == CHardModels::miPropertyLocked
		|| Modelindex == CHardModels::miPickupsave)
		weirdTypeNumber = 0;
	else
	{
		weirdTypeNumber = CPickups::WeaponForModel(Modelindex);
		entity->SetOrientation(90.0*degreetoRadian, 0.0*degreetoRadian, 0.0*degreetoRadian);
